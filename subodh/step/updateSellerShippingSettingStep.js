...
When('User is hitting seller shipping setting for isExcludeShippingAmount value as true', async() => {
...
When('User is hitting seller shipping setting for isExcludeShippingAmount value as false', async() => {
    const input = sellerShipping.input({isExcludeShippingAmount:sellerShippingData.disableExcludeShippingAmount});
    response = await I.sendMutation(sellerShipping.query,input,{},{ token: loginData.token });
});

When('User is hitting seller shipping setting for isExcludeShippingAmount value as null', async() => {
    const input = sellerShipping.input({isExcludeShippingAmount:sellerShippingData.disableExcludeShippingAmount});
    response = await I.sendMutation(sellerShipping.query,input,{},{ token: loginData.token });
});
